...
namespace Mofleet.Authorization.Users
{
    public class UserRegistrationManager : DomainService
    {
        ...
        public async Task<User> RegisterAsyncForUserDriver(string emailAddress, string phoneNumber, string dialCode, string password, Enums.Enum.UserType userType)
        {
            ...
            user.SetNormalizedNames();
            foreach (var defaultRole in await _roleManager.Roles.IgnoreQueryFilters().Where(r => r.Name == StaticRoleNames.Tenants.DriverUser).ToListAsync())
            {
                user.Roles.Add(new UserRole(tenant.Id, user.Id, defaultRole.Id));
            }
            await _userManager.InitializeOptionsAsync(tenant.Id);
