...
namespace Mofleet.Domain.RegisterdPhoneNumbers
{
    public class RegisterdPhoneNumberManager : DomainService, IRegisterdPhoneNumberManager
    {
        ...
        public async Task<bool> CheckVerificationCodeIsValidAsync(string dialCode, string phoneNumber, string verificationCode)
        {
            ...
            if (phoneNumberCode is not null && phoneNumberCode.CreationTime.ToUniversalTime().AddMinutes(MofleetConsts.VerificationTimeCodeInMinutes) > DateTime.UtcNow && !phoneNumberCode.LastModificationTime.HasValue)
            {
                return true;
            }
                ...
                && phoneNumberCode.LastModificationTime?.ToUniversalTime().AddMinutes(MofleetConsts.VerificationTimeCodeInMinutes) > DateTime.UtcNow)
            {
                return true;
            }
            else
