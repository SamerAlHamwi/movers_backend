...
namespace Mofleet.Domains.UserVerficationCodes
{
    public class UserVerficationCodeManager : DomainService, IUserVerficationCodeManager
    {
        ...
        public async Task<bool> CheckVerificationCodeIsValidAsync(long UserId, ConfirmationCodeType confirmationCodeType, bool IsForEmail = false)
        {
            ...
            if (code.CreationTime.ToUniversalTime().AddMinutes(MofleetConsts.VerificationTimeCodeInMinutes) > DateTime.UtcNow && !code.LastModificationTime.HasValue)
            {
                return true;
            }
            else if (code.LastModificationTime.HasValue && code.LastModificationTime.Value.ToUniversalTime() > code.CreationTime.ToUniversalTime() && code.LastModificationTime?.ToUniversalTime().AddMinutes(MofleetConsts.VerificationTimeCodeInMinutes) > DateTime.UtcNow)
            {
                return true;
            }
            else
