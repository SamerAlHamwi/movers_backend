    ...
    public class OfferManager : DomainService, IOfferManager
    {
        ...
        public async Task<OfferDetailsDto> GetOfferDetailsDtoById(Guid id, long userId)
        {
            try
            {
                ...
                if (offer.DriverId.HasValue)
                {
                    var driver = await _driverManager.GetEntityByIdAsync(offer.DriverId.Value);
                    offerDetails.Driver = _mapper.Map<LiteDriverDto>(driver);
                }
                if (offer.TruckId.HasValue)
                {
                    var Truck = await _truckManager.GetEntityByIdAsync(offer.TruckId.Value);
                    offerDetails.Truck = _mapper.Map<LiteTruckDto>(Truck);
                }
